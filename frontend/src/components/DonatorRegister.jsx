...
import { registerDonator, loginWithGoogle, loginWithFacebook } from '../services/api';
...
const DonatorRegister = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await registerDonator({
      ...
      if (response && response.success) {
        ...
        setTimeout(() => {
          // If user data is returned, redirect to donor dashboard
          if (response.user && response.user.role === 'DONOR') {
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <form className="mt-8 space-y-6 bg-white p-8 rounded-xl shadow-lg" onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Professional Field */}
            <div className="md:col-span-2">
              <label htmlFor="professionalField" className="block text-sm font-medium text-gray-700 font-vazir mb-2 text-right">
                حوزه حرفه‌ای شما
              </label>
              <input
                ...
                type="text"
                className="w-full px-4 py-3 border border-light-blue rounded-xl shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue focus:border-blue font-vazir text-right"
                placeholder="حوزه حرفه‌ای خود را وارد کنید"
              />
            </div>
          ...
          <div>
            <button
              ...
              className="w-full flex justify-center items-center py-3 px-4 border border-blue rounded-xl text-blue font-vazir font-medium transition-all duration-200 hover:bg-blue hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue shadow-sm hover:shadow-md"
            >
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600 font-vazir">
              ...
              <button
                type="button"
                onClick={() => navigate('/login', { state: { from: 'donator' } })}
