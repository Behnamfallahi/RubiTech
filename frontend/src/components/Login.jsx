...
const Login = () => {
  ...
  const redirect = (searchParams.get('redirect') || '').toLowerCase();
  const redirectToRegisterMap = {
    student: '/student/register',
    ambassador: '/ambassador/register',
    donor: '/register/donator'
  };
  const registerFallback = '/student/register';
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Enforce role-based redirect for general login
      ...
      // Debug: verify token payload id matches stored userId to prevent cross-user mixups
      try {
        const payload = jwtDecode(token);
        // eslint-disable-next-line no-console
        console.log('[Login] token payload:', payload);
        if (payload && typeof payload === 'object' && String(payload.id) !== String(user.id)) {
          // eslint-disable-next-line no-console
          console.error('[Login] token id does not match user.id – clearing session');
          localStorage.removeItem('authToken');
          localStorage.removeItem('userType');
          localStorage.removeItem('userEmail');
          localStorage.removeItem('userName');
          localStorage.removeItem('userId');
          toast.error('اختلال در نشست – لطفاً دوباره وارد شوید');
          return;
        }
      } catch (_) {}
      ...
      if (error.response?.status === 403) {
        ...
        // Only show invalid credentials when backend explicitly says so
        const msg = error.response?.data?.message || 'ایمیل یا رمز عبور نادرست است';
        toast.error(msg);
      ...
      <div className="w-full max-w-md space-y-6 sm:space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-xs sm:text-sm text-gray-600 font-vazir">
            ایمیل یا شماره موبایل و رمز ورود خود را وارد کنید
          </p>
        </div>
        ...
        <form className="mt-6 sm:mt-8 space-y-4 sm:space-y-6 bg-white p-5 sm:p-8 rounded-xl shadow-lg" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="text-center">
            <p className="text-xs sm:text-sm text-gray-600 font-vazir">
              حساب ندارید؟{' '}
              <button
                type="button"
                onClick={() => navigate(redirectToRegisterMap[redirect] || registerFallback)}
                className="font-medium text-blue-600 hover:text-blue-500 transition-colors duration-200 touch-manipulation"
              >
                ثبت‌نام کنید
              </button>
            </p>
